...
def encoder(x, keep_prob=0.5, isTrain=True):
    with tf.variable_scope("encoder", reuse=tf.AUTO_REUSE):  # 64 * 64 * 4
        conv1 = tf.layers.conv2d(x, 128, [4, 4], strides=(2, 2), padding='same', kernel_initializer=tf.contrib.layers.xavier_initializer())  # 32 * 32 * 128
        lrelu1 = tf.nn.elu(conv1)

        conv2 = tf.layers.conv2d(lrelu1, 256, [4, 4], strides=(2, 2), padding='same', kernel_initializer=tf.contrib.layers.xavier_initializer())  # 16 * 16 *256
        lrelu2 = tf.nn.elu(tf.layers.batch_normalization(conv2, training=isTrain))

        conv3 = tf.layers.conv2d(lrelu2, 512, [4, 4], strides=(2, 2), padding='same', kernel_initializer=tf.contrib.layers.xavier_initializer())  # 8 * 8 * 512
        lrelu3 = tf.nn.elu(tf.layers.batch_normalization(conv3, training=isTrain))

        conv4 = tf.layers.conv2d(lrelu3, 1024, [4, 4], strides=(2, 2), padding='same', kernel_initializer=tf.contrib.layers.xavier_initializer())  # 4 * 4 * 1024
        lrelu4 = tf.nn.elu(tf.layers.batch_normalization(conv4, training=isTrain))

        conv5 = tf.layers.conv2d(lrelu4, 32, [4, 4], strides=(1, 1), padding='valid', kernel_initializer=tf.contrib.layers.xavier_initializer())  # 1 * 1 * 32
...
def generator(x, isTrain=True):
    with tf.variable_scope('generator', reuse=tf.AUTO_REUSE):
        # 1st hidden layer
        conv1 = tf.layers.conv3d_transpose(x, 256, [2, 2, 2], strides=(1, 1, 1), padding='valid', use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())  # (-1, 2, 2, 2, 256)
        lrelu1 = tf.nn.elu(tf.layers.batch_normalization(conv1, training=isTrain))

        # 2nd hidden layer
        conv2 = tf.layers.conv3d_transpose(lrelu1, 128, [4, 4, 4], strides=(2, 2, 2), padding='same', use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer()) # (-1, 4, 4, 4, 128)
        lrelu2 = tf.nn.elu(tf.layers.batch_normalization(conv2, training=isTrain))

        # 3rd hidden layer
        conv3 = tf.layers.conv3d_transpose(lrelu2, 64, [4, 4, 4], strides=(2, 2, 2), padding='same', use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())  # (-1, 8, 8, 8, 64)
        lrelu3 = tf.nn.elu(tf.layers.batch_normalization(conv3, training=isTrain))

        # 4th hidden layer
        conv4 = tf.layers.conv3d_transpose(lrelu3, 32, [4, 4, 4], strides=(2, 2, 2), padding='same', use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())  # (-1, 16, 16, 16, 32)
        lrelu4 = tf.nn.elu(tf.layers.batch_normalization(conv4, training=isTrain))
...
def discriminator(x, isTrain=True):
    with tf.variable_scope('discriminator', reuse=tf.AUTO_REUSE):  # (-1, 32, 32,, 32, 1)
        # 1st hidden layer
        conv1 = tf.layers.conv3d(x, 128, [4, 4, 4], strides=(2, 2, 2), padding='same', use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())  # (-1, 16, 16, 16, 128)
        lrelu1 = tf.nn.elu(conv1)
        # 2nd hidden layer
        conv2 = tf.layers.conv3d(lrelu1, 256, [4, 4, 4], strides=(2, 2, 2), padding='same', use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())  # (-1, 8, 8, 8, 256)
        lrelu2 = tf.nn.elu(tf.layers.batch_normalization(conv2, training=isTrain))

        # 3rd hidden layer
        conv3 = tf.layers.conv3d(lrelu2, 512, [4, 4, 4], strides=(2, 2, 2), padding='same', use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())  # (-1, 4, 4, 4, 512)
        lrelu3 = tf.nn.elu(tf.layers.batch_normalization(conv3, training=isTrain))

        # output layer
        conv4 = tf.layers.conv3d(lrelu3, 1, [4, 4, 4], strides=(1, 1, 1), padding='valid', use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())
...
# load dataset
image_path = "./test_chair_image/"
model_path = "./test_chair_model/"
root = "./result_test_chair/"
